...
function add_card(cards) {
    const card = {
        ...
        blocked: false,
        last_event: random_date_between(moment("20180102", "YYYYMMDD")),
    };
    cards.push(card);
...
function update_card(cards) {
    ...
    card.last_event = random_date_between(card.last_event);
...
export function gen_card_events() {
    ...
    const add_n = random_int_between(1,5);
    const update_n = random_int_between(0,5);
    const cancel_n = random_int_between(0, add_n);
